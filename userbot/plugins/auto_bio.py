...
from userbot.utils import flaming_cmd, edit_or_reply

DEFAULTUSERBIO = str(BIO_MSG) if BIO_MSG else "FLAMING"
DEL_TIME_OUT = 60
DEFAULTUSER = str(ALIVE_NAME) if ALIVE_NAME else "FLAMING"


@bot.on(flaming_cmd(pattern="autoname"))  # pylint:disable=E0602
async def _(event):
...
@bot.on(flaming_cmd(pattern="autobio"))  # pylint:disable=E0602
async def _(event):
    if event.fwd_from:
        return
    while True:
