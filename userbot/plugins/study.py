...
@borg.on(flaming_cmd(pattern=r"study ?(.*)"))
async def _(event):
    if event.fwd_from:
        return
    global USER_night  # pylint:disable=E0602
    ...
    if not USER_night:  # pylint:disable=E0602
        ...
        if reason:
            await event.edit(f"My Boss Is Going To study  Dnd  ")
        else:
            await event.edit(f"My Boss is Going To study")
...
async def on_night(event):
    ...
    if USER_night and not (await event.get_sender()).bot:  # pylint:disable=E0602
        ...
        message_to_reply = (
            ...
            if reason
            else f"**Important Notice**\n\n[{DEFAULTUSER} is studying hard and you should also study to gain success...](https://telegra.ph/file/2512147bc1e2d4ae27922.mp4) "
        )
