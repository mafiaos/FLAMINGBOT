...
@borg.on(flaming_cmd(pattern="(logs|log)"))
...
@borg.on(flaming_cmd(pattern="(rerun|restarts)"))
...
@borg.on(flaming_cmd(pattern="usage$"))
...
@borg.on(
    flaming_cmd(pattern="(set|get|del) var(?: |$)(.*)(?: |$)([\s\S]*)", outgoing=True)
)
@borg.on(
...
@borg.on(flaming_cmd(pattern="shp ?(.*)"))
async def lel(event):
...
@borg.on(flaming_cmd(pattern="acolb (.*)"))
async def sf(event):
...
@borg.on(flaming_cmd(pattern="tfa (.*)"))
async def l(event):
    hmm = event.pattern_match.group(1)
    app = Heroku.app(Var.HEROKU_APP_NAME)
