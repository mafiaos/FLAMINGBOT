...
import asyncio
from userbot.events import flaming_cmd
config=Config
...
GIT_REPO_NAME = "FLAMING"
UPSTREAM_REPO_URL = "https://github.com/ITS-FLAMINGBOT/FLAMINGBOT"
...
@borg.on(flaming_cmd(pattern="update ?(.*)"))
async def upstream(ups):
    ...
    if force_updateme:
        ...
        await ups.edit('`Updating your` FLAMING USERBOT `please wait for a while`')
        await asyncio.sleep(4)
        await ups.edit('Updated Sur✨\nRestarting it please have patience and enjoy botless life for a while\nIncase restart structs join [FLAMING SUPPORT](https://t.me/flamingchat) ')
    ...
    if config.HEROKU_API_KEY is not None:
        ...
        await ups.edit('`Sync Verified Successfully 🙂🙂\n'
                       'Restarting, please wait a min ,then type .alive to check if I alive !! Else Go to [FLAMING SUPPORT](https://t.me/flamingchat)`')
    else:
